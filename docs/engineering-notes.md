...
검토 payload의 `passed`도 에이전트가 잘못 선언할 수 있다. 저장 시 raw에서 현재
챕터 번호와 일치하는 첫 줄 제목·반복 제목·연속 번호형 목차·확인된 부모의 하위 번호를 보수적으로
찾아 inventory와 직접 대조한다. 확실한 번호 제목·순서·계층이 하나라도 다르면 완료로 저장하지
않는다. 챕터 번호를 제목에서 확정할 수 없는 서문은 책 전체 목차 조각을 현재 구조로
오인하지 않도록 이 로컬 검증을 생략하고 전체 원문 검토에 맡긴다. 이 검증은 텍스트
목차로 챕터 경계를 추정하는 기능이 아니다.
