...
- 텍스트 품질 점수(`high`/`medium`/`low`/`no_text_layer`/`garbled`)는 `scan_pdf`가
  `scan_size`(기본 30)p 샘플로 계산해 `state.text_quality`에 저장한다(이 샘플 텍스트는
  언어 감지에도 재사용). 이 값은 **챕터 분리(목차)
...
> 텍스트 기반 목차 정규식 파싱은 **제거됐다** — 스캔본·부분 모지바케에서
> 제목↔페이지번호 정렬이 깨져 잘못된 챕터가 나오던 원인이었다. 챕터 경계는 내장
> 목차(정확) 또는 목차 이미지(vision)로만 정한다.
