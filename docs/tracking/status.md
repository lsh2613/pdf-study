...
- text 모드는 챕터 본문을 서버가 추출하고, OCR 모드는 페이지 이미지를 제공한 뒤 요약자가 `body_text`를 보내면 raw 본문으로 되돌려 저장한다.
...
- 테스트 모음은 PDF 스캔, 챕터 경계 추천, OCR 입력, raw 본문 되돌림, 서버 응답 봉투, 선택지 요구, 최종 렌더링, 진도 저장 서버, 설치 스크립트를 다룬다.
...
- OCR 모드에서 `body_text` 누락을 저장 경계가 직접 거부하도록 보강할 필요가 있다. 현재는 프롬프트가 요구하고 저장기는 값이 있을 때 되돌려 저장한다.
