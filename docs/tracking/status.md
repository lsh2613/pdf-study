...
- 최근 확인: `.venv/bin/python -m pytest -q`가 223개 테스트를 모두 통과했다. 경고는 PyMuPDF/Paddle 하위 SWIG 타입의 DeprecationWarning 5개다.

## 남은 일

- 실제 대형 스캔본에서 OCR 모드 CPU 사용량과 처리 시간을 줄이는 배치 전략을 더 정교하게 만들 수 있다.
- 확장 문제 작성자는 챕터에서 고른 키워드나 주제 수준 검색어를 쓰고, PDF 본문 전체나 긴 원문 단락을 검색어로 보내지 않는 기준을 계속 지켜야 한다.
