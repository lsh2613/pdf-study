...
- 요약 전에 전체 본문의 `content_map`을 만들고, 명시적인 서브 챕터 제목과 모든
  중요 항목을 보존한다. 원문·content map·초안을 대조한 검토가 전체 coverage와
  중요 누락·왜곡 부재를 확인한 경우에만 완료로 저장하며 고정 글자 수 기준은 쓰지
  않는다.
...
- 최근 확인: 현재 checkout의 프로젝트 `.venv`에서
  `.venv/bin/python -m pytest -q`로 373개 테스트가 모두 통과했다. 경고는
