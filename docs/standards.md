...
OCR 모드에서 서버는 `set_chapters` 반환 전에 non-skip 챕터의 페이지 이미지를 PaddleOCR CPU로 읽고 raw 본문을 저장해야 한다. 하나의 챕터 안에서는 페이지 순서를 지키고, 여러 챕터만 제한된 worker 수로 병렬 처리한다. 페이지 OCR 예외나 챕터 전체 공백 결과가 있으면 partial raw 본문을 저장하지 말고 실패 상태와 오류를 남겨야 한다.
