...
`get_subagent_prompts`는 요약과 확장 결과의 실제 pending 챕터 목록을 분리해 반환해야 한다. 호환용 `chapter_ids`는 두 목록의 자연 정렬 합집합만 담고, raw 검증과 workflow·`next_action`은 완료 챕터를 제외한 이 처리 대상과 실제 pending 결과 유형에만 적용해야 한다.
