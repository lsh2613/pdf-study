...
렌더러는 비어 있는 staging 폴더에 한 세대를 완성하고, 최종 폴더 교체는 `.pdf-study-manifest.json`에 기록된 관리 경로에만 적용해야 한다. manifest 밖의 사용자 파일을 삭제하거나 덮어쓰면 안 되며, 실패하면 이전 세대와 manifest를 복원해야 한다.
