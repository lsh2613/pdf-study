...
### F-007 [해결: 2026-07-21, 변경: 2026-07-23] MCP 설정의 실제 클라이언트 형식과 범위를 맞춘다

- 결정: MCP 클라이언트 설정은 모두 사용자 전역 범위로 통일한다. `--global`은 기존
  호출 호환을 위해 허용하고, `--local`은 설정 범위를 오해하지 않도록 거부한다.
- 구현: Claude Code와 Antigravity CLI는 전역 JSON 설정을 검증·백업·원자 교체한다.
  Codex CLI는 JSON 파일을 직접 만들지 않고 공식 `codex mcp add`로 전역
  `pdf-study` 서버를 등록한 뒤 `codex mcp get pdf-study`로 확인한다.
- 보호: JSON 설정은 대상 전체를 먼저 검증하므로 손상된 설정을 덮어쓰지 않는다.
  기존 JSON을 변경할 때는 `.pdf-study.bak` 백업과 임시 파일·fsync·원자 교체를 쓴다.
- 검증: 전역 경로만 사용하는지, 로컬 범위를 거부하는지, 손상 JSON 무변경 실패,
  기존 JSON 백업·원자 교체, Codex CLI 등록·조회 명령 구성을
  `tests/test_mcp_config.py`로 확인했다.
...
- stale 방지: 현재 상태가 `completed`인 요약·문제 JSON만 렌더 입력으로 읽으므로
  부분 렌더링도 pending 챕터의 같은 ID 예전 파일을 포함하지 않는다.
...
### F-015 [해결: 2026-07-23] OCR 언어가 한국어 모델 하나로 고정되어 있다

- 결정: OCR이 필요한 목차 이미지 흐름과 텍스트 레이어가 없거나 깨진 PDF에서
  `scan_pdf`가 한국어·영어 선택지를 반환한다. `prepare_ocr`는 사용자가 고른 언어의
  모델을 준비하고 작업 상태에 보존한다.
- 처리 경계: 지원 언어는 `korean`, `english`뿐이다. OCR raw에는 사용 언어를 기록해
  다른 언어로 다시 설정한 본문 OCR이 이전 raw를 재사용하지 않게 했다.
- 검증: 언어 선택 응답, 영어 모델 선택, 준비 성공 후 상태 저장, 기존 OCR·서버·통합
  회귀를 테스트로 확인했다.

### F-016 [해결: 2026-07-23] Markdown+TUI가 저장된 답안부터 다시 묻는다

- 결정: `progress.json.answers`에 답안이 있고 완료되지 않은 챕터는 메뉴 선택 없이
  문제 순서상 첫 미응답 문제부터 자동으로 이어서 푼다.
- 처리 경계: 이미 저장된 문제는 다시 묻지 않고, 완료된 챕터의 기존 메뉴 동작은
  바꾸지 않는다.
- 검증: 진행 중인 챕터에서 메뉴를 건너뛰는 동작과 저장된 문제를 건너뛰는 동작을
  템플릿 엔진 테스트로 확인했다.

### F-017 [해결: 2026-07-23] 문제 JSON의 ID와 최대 개수가 프롬프트에만 의존한다

- 결정: `question_contract.py`를 문제 ID 문자·챕터 내 유일성·본문 글자 수별 최대
  개수의 단일 규칙으로 확장했다. 문제 ID는 영문자·숫자·`_`·`-`만 허용한다.
- 저장 경계: 기본·확장 저장 도구가 각각 raw의 `char_count`로 유형별 상한을 검사한다.
  같은 챕터의 다른 결과 유형이 병렬 저장될 때도 workspace 잠금 안에서 ID 충돌을 다시
  검사하므로, 먼저 도착한 파일을 다음 저장이 덮어쓰거나 HTML/TUI 답안 키가 충돌하지
  않는다.
- 검증: 안전하지 않은 ID, 기본 유형 간 중복, 짧은 챕터 상한 초과, 확장 문제와 기본
  문제 사이의 중복을 회귀 테스트로 확인했다.

### F-018 [해결: 2026-07-23] 부분 렌더링에 별도 `force` 호출이 필요하다

- 결정: `finalize_study`는 미완료 결과가 있어도 완료분으로 최종 자료를 만든다.
  `force` 파라미터를 제거해 별도 강제 호출과 사용자 동의 선택 단계를 없앴다.
- 안내: 성공 응답의 `omitted_chapters`와 실행 안내가 미반영 챕터별 결과 종류,
  `pending`·`in_progress`·`failed` 상태, 저장된 오류를 알린다. 완료가 아닌 결과나
  예전 JSON은 자료에 포함하지 않는다.
- 검증: 실패한 챕터를 제외한 결과 생성, 누락 정보 반환, `force` 서명 제거를 회귀
  테스트로 확인했다.
