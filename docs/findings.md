...
- 검증: 오래된 두 경로가 다시 주석에 들어오지 않고 현존 문서 두 개를 참조하는지
  `tests/test_md_tui_renderer.py`에서 확인했다.
...
5. 완료 — F-005, F-006, F-007로 fixture와 설치·검증·MCP 설정 적용 절차를 재현
   가능하게 만들었다.
6. F-004, F-008 완료 — 처리 모드 선택지의 자동 추천 표현과 오래된 문서 참조를
   정리했다. 이후 나머지 선택 흐름과 중복 파일을 작은 리팩터링 단위로 정리한다.
