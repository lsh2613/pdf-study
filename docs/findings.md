...
### F-002 [P1] `printed_range`를 보존한다는 계약과 달리 챕터 확정 시 값이 사라진다

- 발생 조건: `scan_pdf`가 만든 `printed_range`를 포함해 `set_chapters`를 호출한다.
- 관찰 증상: `analysis._validate_chapter_def`는 값을 보존하지만
  `workspace.set_chapters_in_state`가 상태에 기록하지 않는다. raw 데이터와
  `set_chapters` 응답도 이 값을 이어받지 않으며 두 렌더러는 물리 PDF 페이지만
  표시한다.
- 문서와의 차이: `server.set_chapters` 설명은 `printed_range`를 표시용 메타로
  보존한다고 하고, `scan_pdf` 안내는 PDF 페이지와 책 페이지를 함께 보여주도록
  요구한다.
- 영향: 사용자가 확인한 책 페이지 정보가 확정 직후 유실되어 최종 자료에서는
  `p.N-M`이 PDF 물리 페이지인지 책 페이지인지 알기 어렵다.
- 가능한 접근: 상태·raw·렌더 중립 데이터에 `printed_range`를 유지하고 HTML/TUI에
  두 페이지 체계를 명확히 표시한다.
...
### F-009 [P1] 같은 출력 폴더 재사용 시 이전 작업과 새 작업의 파일이 섞인다

- 발생 조건: 같은 PDF 기본 경로 `result/<pdf_basename>`로 다시 시작하거나, 같은
  작업을 HTML과 Markdown+TUI로 번갈아 렌더링하거나, 챕터 수를 줄여 재렌더링한다.
- 관찰 증상: `create_workspace`는 디렉터리와 `state.json`만 만들거나 덮어쓰며 이전
  raw/요약/퀴즈/렌더 결과를 정리하지 않는다. 렌더러도 이전 포맷 파일,
  사라진 `chN.html`/`chN/`, 이전 `progress`를 제거하지 않는다. `force=true`에서는
  현재 챕터와 ID가 같은 예전 JSON을 읽을 수도 있다.
- 영향: 새 학습 자료에 예전 요약·문제·진도가 나타나거나 한 출력 폴더에 두 포맷이
  혼재한다.
- 가능한 접근: 새 작업은 충돌 없는 디렉터리를 만들거나 기존 작업을 명시적으로
  resume/replace하도록 선택시킨다. 렌더는 관리 manifest를 기준으로 원자적으로
  교체하고 progress는 PDF/챕터 fingerprint가 같을 때만 재사용한다.
...
2. F-009와 F-010으로 작업 간 데이터 혼재 가능성을 제거한다.
3. F-002, F-003, F-011로 상태·재개 계약을 맞춘다.
4. F-005, F-006, F-007로 설치와 검증 절차를 재현 가능하게 만든다.
5. 나머지 선택 흐름과 중복 파일을 작은 리팩터링 단위로 정리한다.
