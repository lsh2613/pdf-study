...
기본 문제(객관식, 단답형, 주관식)는 의미 보존 검토를 통과한 현재 챕터의 `summary`와 `key_points`만으로 정답과 해설 또는 모범 답안을 유추하고 검증할 수 있어야 한다. 동시에 문제 문장 하나만 읽어도 묻는 실제 개념·대상·상황과 필요한 조건을 알 수 있어야 하며, 학습자가 요약을 열어 문제의 문맥을 보충하게 만들면 안 된다. `챕터 핵심 내용`, `핵심 원칙`, `이 챕터`, `위 내용`처럼 앞선 문맥만 가리키는 일반 템플릿은 금지한다. 먼저 원문 구조 inventory와 요약을 만들고 검토한 뒤, 별도 문제 생성 단계에는 원문과 section inventory를 전달하지 않는다. 외부 지식, 일반 상식, 다른 챕터 내용, 요약에 없는 원문 세부 정보가 있어야 풀 수 있는 문제는 만들면 안 된다. 주관식도 개인 의견형 토론이 아니라 요약 근거를 확인하는 검증형 문제여야 한다. 객관식에서 생성 agent는 정답 하나와 오답 보기를 만들고, 서버는 결과 저장 시에만 보기 순서와 정답 위치를 배치한다. 저장된 보기 순서와 정답 위치는 렌더·재개·재최종화에서 바뀌지 않는다.
...
확장 문제는 외부 검색이나 별도 자료 수집 없이 저장된 해당 챕터의 `summary`, `key_points`와 학습자 정보만으로 만든다. `get_chapter_summary`는 문제 생성에 원문을 다시 전달하지 않으며, 원문 글자 수는 문제 개수 상한 계산에만 사용한다. 현실·실무 맥락이 필요하면 문제 안에 필요한 상황과 조건, 적용할 개념을 충분히 제시해 문제 문장만으로 의도를 파악할 수 있게 만들며, 최신 외부 사실이나 별도 출처를 알아야만 답할 수 있는 문제는 만들지 않는다.
