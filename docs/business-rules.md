...
기본 문제(객관식, 단답형, 주관식)는 현재 챕터 본문만으로 정답과 해설 또는 모범 답안을 유추하고 검증할 수 있어야 한다. 외부 지식, 일반 상식, 다른 챕터 내용, 보이지 않는 이미지 정보가 있어야 풀 수 있는 문제는 만들면 안 된다. 주관식도 기본 문제인 한 개인 의견형 토론이 아니라 본문 근거를 확인하는 검증형 문제여야 한다. 객관식에서 생성 agent는 정답 하나와 오답 보기를 만들고, 서버는 결과 저장 시에만 보기 순서와 정답 위치를 배치한다. 저장된 보기 순서와 정답 위치는 렌더·재개·재최종화에서 바뀌지 않는다.
