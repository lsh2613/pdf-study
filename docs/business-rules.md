...
챕터 경계의 정상 출처는 PDF 내장 목차와 목차 페이지 이미지뿐이다. 내장 목차가 있으면 그 물리 페이지를 우선 후보로 삼고 사용자에게 확인한다. 내장 목차가 틀렸거나 없으면 서버가 목차 페이지 이미지를 렌더하고 PaddleOCR CPU로 읽은 `ocr_text`를 함께 제공하며, 클라이언트는 그 텍스트와 필요 시 이미지를 확인해 챕터를 구성한다.
