...
이 명령은 `.venv` 생성, 패키지 설치, import 검증을 실행한 뒤 Claude Code, Codex CLI, Antigravity CLI 설정을 자동 적용한다. 로컬 설정은 저장소 루트 기준으로 적용하므로 저장소 밖에서 스크립트를 호출해도 clone한 저장소에 기록된다. 대상 클라이언트를 지정하지 않으면 세 클라이언트 설정을 모두 적용한다.

기존 설정이 손상된 JSON이거나 예상한 객체 구조가 아니면 기존 파일을 덮어쓰지 않고 중단한다. 기존 설정을 갱신할 때는 `.pdf-study.bak` 백업을 만든 뒤 원자적으로 교체한다.
