...
분리 workflow에서 학습자 정보는 문제의 난이도, 표현, 예시, 관점에만 영향을 준다.
section inventory, 요약, 의미 보존 검토에는 전달하지 않는다. 호환용 결합 프롬프트는
전체 원문 요약을 먼저 확정한 뒤 문제 단계에만 적용한다. 어느 경로에서도 학습자
정보와 겹치는 내용만 요약하는 필터로 쓰지 않는다.
...
- 명시적인 하위 절이 있으면 번호 유무와 계층 깊이에 관계없이 모든 제목·순서·
  상대 계층을 유지한다.
- 명시적인 하위 절이 없으면 챕터 전체를 하나의 section으로 보고, 원문에 없던
  하위 구조를 만들지 않는다.
...
- 의미 보존 검토를 통과한 현재 챕터의 summary와 key_points만으로 답과 해설을
  유추하고 검증할 수 있다.
- 문제 생성 단계에는 Raw 본문과 section inventory를 전달하지 않는다.
...
2. 요약 pending이면 챕터 본문으로 section inventory를 만든 뒤 각 section 전체를
   요약하고 원문과 대조 검토한다.
