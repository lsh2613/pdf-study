...
- 챕터별 한국어 요약
- 기억해야 할 핵심 포인트
- 검토된 요약 근거로 정답을 검증할 수 있는 기본 문제
- 요약 개념을 현실·실무 맥락으로 확장하는 선택적 문제
...
5. 각 챕터의 생성 결과는 저장 전에 완전성과 요약 근거 규칙을 검증한다.
...
- 챕터 본문으로 요약을 생성·검토하고, 검토된 요약만으로 문제 생성
- 순차 또는 병렬 처리 지시 준수
...
새 작업을 실제로 만들기 전에 단일 필드 form을 다음 순서로 받는다.

| 값 | 의미 |
| --- | --- |
| 단답형 포함 여부 | 짧은 문장으로 답하는 본문 검증 문제 |
| 주관식 포함 여부 | 요약 근거를 설명하는 서술형 검증 문제 |
| 확장형 포함 여부 | 본문 개념을 현실·실무 맥락에 적용하는 문제 |
| 학습자 정보 | 학습 목적, 배경지식, 관심 분야, 현재 수준을 담는 필수 문자열 |

객관식은 제품의 기본 학습 검증 유형으로 항상 활성화한다. 따라서 다른 문제 유형을
모두 제외해도 학습자 정보 form을 네 번째로 연다. 이 form은 `required`와
`minLength=1`을 사용하고, 서버도 trim 후 공백뿐인 값을 거부한다. 사용자가 모든
form을 승인하고 유효한 학습자 정보를 제공하기 전에는 작업 폴더와 상태를 만들지
않는다. 기존 작업의 미확정 설정을 보완하는 `scan_pdf`는 이전 작업과의 호환성을 위해
선택 입력을 유지한다.
...
책 정보는 원자료에 대한 메타데이터다. 학습자 정보는 사용자에 대한 학습 맥락이며 새
작업 생성에는 필수다. 두 값을 섞지 않는다. 기존 작업에 저장된 빈 학습자 정보는
소급 마이그레이션하지 않는다.
...
학습자 정보는 난이도, 표현, 예시, 문제 관점에만 영향을 주며 요약 근거 제한을
완화하지 않는다.
...
- 요약 전에 챕터 전체에서 유의미한 내용을 `content_map`으로 정리한다.
- 명시적인 하위 절이 있으면 모든 하위 절을 content map section으로 보존하고,
  최종 Markdown에도 각 제목을 직접 표시한다.
- 요약 뒤 원문·content map·초안을 대조해 중요한 누락이나 왜곡이 있으면 보완한다.
...
요약 근거가 부족하면 더 적게 만든다. 상한을 채우려고 중복, 사소한 질문, 약한
...
- 의미 보존 검토를 통과한 현재 챕터의 summary와 key_points만으로 답과 해설을
  유추하고 검증할 수 있다.
- 문제 생성 단계에는 Raw 본문과 content map을 전달하지 않는다.
- 다른 챕터를 읽어야만 풀 수 없어야 한다.
- 외부 지식이나 일반 상식을 알아야만 풀 수 없어야 한다.
- 보이지 않는 이미지의 시각 정보를 요구하지 않는다.
- 그림 캡션 내용이 요약에 있어도 그 요약 텍스트만으로 충분히 답할 수 있을 때만
  문제화한다.
...
- `get_chapter_summary`가 반환한 summary, key_points와 학습자 정보만 사용한다.
- Raw 본문은 전달하지 않으며 source_char_count는 문제 개수 상한에만 사용한다.
...
- 비어 있지 않은 `summary`
- 비어 있지 않은 `key_points`
- 전체 챕터에서 만든 유효한 `content_map`
- 모든 section과 important point를 반영하고 중요 누락·왜곡이 없는
  `passed` 상태의 `summary_review`
- `questions` 객체
- 모든 기본 문제 유형 배열
...
2. 요약 pending이면 챕터 본문으로 content map과 요약을 만들고 원문과 대조 검토한다.
3. 통과한 summary, key_points만 별도 기본 문제 생성 단계에 전달해 기본 문제를
   만들고 요약 결과와 함께 저장한다.
4. 확장 pending이면 저장된 요약을 `get_chapter_summary`로 받아 확장 결과를
   생성하고 저장한다.
...
- 한 배치에 최대 5개 챕터
- 챕터별 원문 조회는 요약 pending일 때만 한 번
- 한 챕터 안에서는 요약 검토·기본 문제·요약 저장·확장 문제의 의존 순서를 유지
...
| 역할명 | 주요 입력 | 책임 |
| --- | --- | --- |
| `init_work` | PDF 경로 | 고정 출력 경로 계산, 출력 충돌 확인, 문제 유형·필수 학습자 정보 form, 작업 생성 |
...
```json
{
  ...
  "user_context": "데이터베이스를 처음 배우며 실무 예시를 선호하는 직장인",
  "page_count": 320,
...
9. 외부 검색 없이 검토된 요약과 학습자 정보만으로 확장 문제를 만든다.
